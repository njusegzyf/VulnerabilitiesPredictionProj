...
object Exp1EvalApp extends App {
  ...
  private def evalExp1Category(): Unit = {
    ...
    val rnnData: EncoderStateData[Double] = RnnUtils.parseRnnLogForce(rnnLogLines, Exp1Configs.exp1RnnLayer, Exp1Configs.exp1RnnUnitNum)
    ...
    val evalAlgorithms: Vector[() => AbstractClassifier] = Exp1Configs.getExp1EvalAlgorithms(Feature.Category)
  ...
  private def evalExp1Amount(): Unit = {
    ...
    val evalAlgorithms: Vector[() => AbstractClassifier] = Exp1Configs.getExp1EvalAlgorithms(Feature.Amount)
    for (evalAlgorithm <- evalAlgorithms) {
  ...
  private def evalExp1Impact(): Unit = {
    ...
    val evalAlgorithms: Vector[() => AbstractClassifier] = Exp1Configs.getExp1EvalAlgorithms(Feature.Impact)
    for (evalAlgorithm <- evalAlgorithms) {
