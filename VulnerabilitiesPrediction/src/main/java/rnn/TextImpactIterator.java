 ...
 * @author Alex Black
 */
public class TextImpactIterator implements DataSetIterator {
...
//     */
    public TextImpactIterator(WordVectors wordVectors, int batchSize, int truncateLength) throws IOException, SQLException {
        this.batchSize = batchSize;
        this.vectorSize = wordVectors.getWordVector(wordVectors.vocab().wordAtIndex(0)).length;

        Connection conn = DBConnection.getConnection();
        impacts = RnnData.getImpact(conn);
        summaries = RnnData.getSummary(conn);
        conn.close();
    ...
    private DataSet nextDataSet(int num) throws IOException {
        ...
        List<Integer> impactCat = new ArrayList<>(num);
        ...
        INDArray labels = Nd4j.create(descs.size(), 11, maxLength);    //Two labels: positive or negative
        ...
        for (int i = 0; i < descs.size(); i++) {
            ...
            labels.putScalar(new int[]{i, impactCat.get(i), lastIdx - 1}, 1.0);   //Set label: [0,1] for negative, [1,0] for positive
